...
root = zarr.open(snakemake.output[0], mode='a')
...
for i in range(n_time):
    instr_cube_to_zarr(i, snakemake.output[0], spectra_output_dir, channel)
...
pathlib.Path(snakemake.output[1]).parent.mkdir(parents=True, exist_ok=True)
...
write_overlappogram(overlappogram, snakemake.output[1])
